...
export function BottomNav() {
  ...
  return (
    <nav className={`lg:hidden fixed bottom-4 left-4 right-4 z-10 rounded-2xl transition-colors duration-300 ${darkMode ? 'glass-dark' : 'glass'}`}>
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-all min-w-0 ${isActive ? '' : 'opacity-60'
                }`}
            >
